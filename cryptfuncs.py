...
def main():
    """
    Test the functions in this suite.
    """
    # Generate a new key pair.
    # Note: the .decode() function just converts a string from binary to a regular string.
    public, private = new_rsa(1024)
    print(public.export_key().decode())
    print(private.export_key().decode())
    # Write the keys to text files.
    file_out = open("private.txt", "wb")
    file_out.write(private.export_key())
    file_out = open("public.txt", "wb")
    file_out.write(public.export_key())

    # Function to go from private key to public key:
    print(get_public_key(private).export_key().decode())

    # Encrypt:
    enc = encrypt("Test", public)
    print(enc)
    # Decrypt:
    ...
    # Sign:
    sig = sign("Test", private)
    print(sig)
    # Verify:
    ver = verify("Test", sig, public)
    print("Signiture verifies correctly: {}".format(ver))

    # Import private key from a text file.
    imp = import_key("private.txt")
    print(imp.export_key().decode())

    pubs = [0 for _ in range(10)]
    priv = [0 for _ in range(10)]
    for i in range(10):
        pubs[i], priv[i] = new_rsa(1024)

    s = "A global and yet not secret password"
    s_sign = sign(s, private)
    for i in range(10):
        check = verify(s, s_sign, pubs[i])
        if (check == True):
            print("The first signature matches a key!")
            break
    else:
        print("The first signature doesn't match a key!")

    s_sign2 = sign(s, priv[5])
    for i in range(10):
        check = verify(s, s_sign2, pubs[i])
        if (check == True):
            print("The second signature matches a key!")
            break
    else:
        print("The second signature doesn't match a key!")

    s_encrypted = encrypt(s, public)
    for i in range(10):
        try:
            dec = decrypt(s_encrypted, priv[i])
            if (dec == s):
                print("The first key was in the list of allowed keys!")
                break
        except ValueError:
            pass
    else:
        print("The first key was not in the list of allowed keys.")

    s_encrypted = encrypt(s, pubs[5])
    for i in range(10):
        try:
            dec = decrypt(s_encrypted, priv[i])
            if (dec == s):
                print("The second key was in the list of allowed keys!")
                break
        except ValueError:
            pass
    else:
        print("The second key was not in the list of allowed keys.")
